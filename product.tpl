...
*  @author PrestaShop SA <contact@prestashop.com>
    ...
    *}
    ...
    <style>
        ...
        .block_content .cart-prices .price {
...
/**************************************/	
        ...
        @media screen and (max-width:480px){
            ...
            #right {
                ...
                /*left: 30px;*/
                top: -350px;/*top: -320px;*/
                margin-bottom: -350px;/*margin-bottom: -639px;*/
            }
            #add_to_cart {
                width: 266px;
            }
...
/**************************************/	
    ...
    <div itemscope itemtype="http://schema.org/Product">
        <div class="primary_block row">
            ...
            <div id="left" class="pb-left-column col-xs-12 col-sm-4 col-md-5">
				
                <div class="mobilediv" style="width : 100% ;float : left ; display : none"  >	
                    <h1 itemprop="name" style="font-size:25px; font-weight: bold;">
                        {$product->name|escape:'html':'UTF-8'}
                    </h1>

                    <p id="product_reference"{if empty($product->reference) || !$product->reference} style="display: none;"{/if}>
                       <span class="editable" style="color:#949393; font-family:'Comic Sans MS', 'Comic Sans', 'Lucida Handwriting';" itemprop="sku">{$product->reference|escape:'html':'UTF-8'}</span>
                    </p>
            ...
            <!-- end left infos-->
            <!-- center infos -->
            <div id="right" class="pb-center-column col-xs-12 col-sm-4">
                {if $product->online_only}
                <p class="online_only">{l s='Online only'}</p>
                {/if}
                <h1 class="screendiv" itemprop="name" style="font-size:25px; font-weight: bold;">{$product->name|escape:'html':'UTF-8'}</h1>
                <p class="screendiv" id="product_reference"{if empty($product->reference) || !$product->reference} style="display: none;"{/if}>
                ...
                <form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
                    ...
                    <div class="box-info-product">
                        ...
                        <div class="content_prices clearfix screendiv">
                            {if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
                            <!-- prices -->
                            <div class="price">
                                <p class="our_price_display" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                        ...
                        <div class="product_attributes clearfix">
                            ...
                            <div id="attributes">
                                ...
                                <fieldset class="attribute_fieldset" style="margin-top : 20px">
                                    {assign var="groupName" value="group_$id_attribute_group"}
                                    <div class="attribute_list">
                                        {if $id_attribute_group == 1 or $id_attribute_group == 2 or $id_attribute_group == 7}
                                    ...
                                    <ul id="size_to_pick_list" class="clearfix" >
                                        {assign var="default_sizepicker" value=""}
                                        {foreach from=$group.attributes key=id_attribute item=group_attribute}
                                        <li {if $group.default == $id_attribute} class="selected"{/if}>
                                            <a {if $group.default != $id_attribute} style="  text-decoration : none !important " {else } style="  text-decoration : none !important "{/if}  href="{$link->getProductLink($product)|escape:'html':'UTF-8'}" id="size_{$id_attribute|intval}" name="{$group_attribute|escape:'html':'UTF-8'}" class="size_pick{if ($group.default == $id_attribute)} selected{/if}" title="{$group_attribute|escape:'html':'UTF-8'}" style="  text-decoration : none !important ">
                                                {$group_attribute|escape:'html':'UTF-8'}
                                        </a>
                                    </li>
                                    {if ($group.default == $id_attribute)}
                                    {$default_size = $id_attribute}
                                    {/if}
                                    {/foreach}
                                </ul>
                                <input type="hidden" class="size_pick_hidden" name="{$groupName|escape:'html':'UTF-8'}" value="{$default_size|intval}" />
                                ...
                                <ul id="color_to_pick_list" class="clearfix"> 
                                    ...
                                    {assign var="default_colorpickername" value=""}
                                    {foreach from=$group.attributes key=id_attribute item=group_attribute}
                                    {assign var='img_color_exists' value=file_exists($col_img_dir|cat:$id_attribute|cat:'.jpg')}
                                    <li style="margin-left  : 10px !important ;  {if $group.default == $id_attribute} class="selected"{/if}>
                                        <a href="{$link->getProductLink($product)|escape:'html':'UTF-8'}" id="color_{$id_attribute|intval}" name="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" class="color_pick{if ($group.default == $id_attribute)} selected{/if}"{if !$img_color_exists && isset($colors.$id_attribute.value) && $colors.$id_attribute.value} style="background:{$colors.$id_attribute.value|escape:'html':'UTF-8'};"{/if} title="{$colors.$id_attribute.name|escape:'html':'UTF-8'}">

                                       {foreach $images_combinations as $image_combination}
                                       {if in_array($id_attribute, $image_combination.id_attributes)}
                                       <img class="replace-2x img-responsive" src="{$link->getImageLink($product->link_rewrite, $image_combination.id_image, 'small_default')|escape:'html':'UTF-8'}" alt="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" title="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" itemprop="image" />
                                            {assign var='stop' value=true}
                                            {/if}
                                            {/foreach}

                                            {if !$stop && $img_color_exists}
                                            <img src="{$img_col_dir}{$id_attribute|intval}.jpg" alt="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" title="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" width="200%" height="100%" />
                                            {/if}
                                        </a>
                                    </li>
                                    {if ($group.default == $id_attribute)}
                                    {$default_colorpicker = $id_attribute}
                                    {$default_colorpickername = $colors.$id_attribute.name}
                                    {/if}
                                    {/foreach}
                                    {else}
                                    <!--{assign var="default_colorpicker" value=""}
                                    ...
                                    {foreach from=$imagesref  item=prod}
                                    {*counter*}
                                    <li  style="margin-left  : 10px !important "  class="{if $prod.id_product == $product->id}selected{/if}"  style="{if $prod.id_product == $product->id}border : 1px solid #fff !important; color:#fff ;{/if}" >

                                        <a onclick="javascript:document.location.href = this.href" href="{$link->getProductLink($prod.id_product)|escape:'html':'UTF-8'}" {if $prod.id_product == $product->id} id="color_{$default_colorpicker|intval}" name="{$default_colorpickername|escape:'html':'UTF-8'}" {/if} class="color_pick{if $prod.id_product == $product->id} selected{/if}" title="$prod.name_attribute|escape:'html':'UTF-8'">
                                            <img itemprop="image"  class="replace-2x img-responsive"  src="{$link->getImageLink($prod.link_rewrite, $prod.id_image, 'large_default')|escape:'html':'UTF-8'}" title="{$prod.name_attribute|escape:'html':'UTF-8'}" alt="{$prod.name_attribute|escape:'html':'UTF-8'}"/>
                                        </a>
                                    </li>
                                    {/foreach}
										<strong  style=" float :right ;">
											{$imagesref|count} {l s='Couleurs'}
										</strong> 
                                    {/if}
                                </ul>
                ...
                <div class="product_attributes clearfix">
                    ...
                    {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE}{/if}
                    ...
                    <p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
                        ...
                        <input type="text" name="qty" id="quantity_wanted"  value="{if isset($quantityBackup)}{$quantityBackup|intval}{else}{if $product->minimal_quantity > 1}{$product->minimal_quantity}{else}1{/if}{/if}" />
